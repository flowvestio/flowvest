...
(function(){
  ...
  async function connect(state){
    ...
    // ✅ 不要 window.location.reload()（OKWeb3App 会疯刷）
    if (window.ethereum && !window.__FLOWVEST_LISTENERS__){
      window.__FLOWVEST_LISTENERS__ = true;

      window.ethereum.on("accountsChanged", () => {
        UI.log("Account changed. Please reconnect.");
        disconnect(state);
      });

      window.ethereum.on("chainChanged", async () => {
        UI.log("Network changed. Please reconnect.");
        disconnect(state);
        await refreshNetworkInfo();
      });
    }
